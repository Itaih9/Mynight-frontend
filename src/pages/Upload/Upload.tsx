        ...
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="bg-white rounded-[40px] p-8 md:p-12 w-full max-w-md relative z-10 shadow-2xl text-center max-h-[90dvh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-5 text-right">
                    <div><label className="block text-sm font-bold text-gray-400 mb-2 pr-1">מספר טלפון</label><input type="tel" dir="ltr" className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-black outline-none transition-all text-left font-medium" value={phoneNumber} placeholder="054-7700000" onChange={handlePhoneChange} /></div>
                    <div><label className="block text-sm font-bold text-gray-400 mb-2 pr-1">אימייל</label><input type="email" dir="ltr" className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-black outline-none transition-all text-left font-medium" value={emailAddress} placeholder="name@example.com" onChange={e => { setEmailAddress(e.target.value); if (error) setError(''); }} /></div>
                    <div><label className="block text-sm font-bold text-gray-400 mb-2 pr-1">סיסמה חדשה</label><div className="relative"><input type={showPass ? "text" : "password"} className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-black outline-none transition-all text-right font-medium" value={pass} placeholder="לפחות 6 אותיות באנגלית" onChange={e => setPass(e.target.value)} /><button type="button" onClick={() => setShowPass(!showPass)} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300">{showPass ? <EyeOff size={20} /> : <Eye size={20} />}</button></div></div>
                    <div><label className="block text-sm font-bold text-gray-400 mb-2 pr-1">אימות סיסמה</label><input type={showPass ? "text" : "password"} className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-black outline-none transition-all text-right font-medium" value={confirm} placeholder="הזינו שוב את הסיסמה" onChange={e => setConfirm(e.target.value)} /></div>
